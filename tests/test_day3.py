...
class TestArtifacts:
    ...
    def test_prepare_attachments_always_includes_logs_on_failure(self, tmp_path):
        ...
        to_attach, _ = prepare_attachments(
            artifacts=[], log_files=[log_path], job_failed=True, attach_logs=False,
        )
    ...
    def test_prepare_attachments_logs_skipped_on_success_when_disabled(self, tmp_path):
        ...
        to_attach, _ = prepare_attachments(
            artifacts=[], log_files=[log_path], job_failed=False, attach_logs=False,
        )
    ...
    def test_budget_overflow_compressed_into_zip(self, tmp_path):
        ...
        a1 = tmp_path / "a.csv"; a1.write_bytes(b"x" * (600 * 1024))
        a2 = tmp_path / "b.csv"; a2.write_bytes(b"y" * (600 * 1024))
        log = tmp_path / "stdout.log"; log.write_text("ok")
        arts = [
        ...
        to_attach, skipped = prepare_attachments(
            artifacts=arts, log_files=[str(log)], job_failed=False,
            max_total_mb=1.0, attach_logs=True,
        )
